...
void Matrix_fill_border(Matrix* mat, int value) {
  ...
  int finalColIndex = ((*mat).width * ((*mat).height-1));
  for (int i = finalColIndex; i < (finalColIndex +(*mat).width); i++)
  {
    *Matrix_at(mat,0,i) = value;
  }
...
int Matrix_column_of_min_value_in_row(const Matrix* mat, int row,
  ...
  int minColIndex = column_start;
  for (int j = column_start; j < column_end; j++)
  {
    if(*Matrix_at(mat,row,j) < minValue || (*Matrix_at(mat,row,j) == minValue && j < minColIndex))
    {
      minValue = *Matrix_at(mat,row,j);
      minColIndex = j;
    }
...
int Matrix_min_value_in_row(const Matrix* mat, int row,
  ...
  for (int j = column_start; j < column_end; j++)
  {
    if(*Matrix_at(mat,row,j) < minValue )
    {
      minValue = *Matrix_at(mat,row,j);
    }
